...
I've never seen it more than 8 characters. Buffer can be increased here, though.
$classname = New-Object System.Text.StringBuilder $classlength # will hold the
class name
$Return = [CustomNameSpace.advapi32]::RegQueryInfoKey(
$Return = [CustomNameSpace.advapi32]::RegQueryInfoKey(
$this.Handle,
...
Write-Debug $datetimemfunction set_smdir {
if(!(test-path("$smdir"))){
mkdir $smdir
}
}
...
# setting printer logs to enabled
...
#### printer queue stuff thats sorta dangerous because if i ts running while
powershell is being patched will send computer into massive resource loop that grinds
machines to a halt
...
function clean_up_start_files {
$paths_to_test = '*.nocomputer','*.noconfig','*.fail'
$paths_to_test | foreach {
$path = Join-Path -Path $smdir -ChildPath $PSItem
...
# read server deploy config and inser t its config into the current active config
...
Start-Process powershell.exe -Argument " -file $smdir\script-monitor.ps1"
