...
$path = "\\$upserver\c$\ProgramData\1E\NomadBranch\$pkgid" + "_Cache"
...
if(!(test-path("\\$upserver\c$\ProgramData\1E\NomadBranch\skpswi.dat"))){

$server | out-file -Append $badlog\$datetime-skpswi-missing.log
...
if(!(test-path("\\$fserver\c$\upload"))){

mkdir \\$fserver\c$\upload
...
"powershell.exe -file c:\upload\ncm.ps1 $pkgid" | out-file c:\temp\ncm.bat

get-content c:\temp\ncm.bat | set-content \\$fserver\c$\upload\ncm.bat

 

if(test-path("\\$fserver\c$\upload\ncm.ps1")){

remove-item \\$fserver\c$\upload\ncm.ps1

}

# create the ps file to run locally on the remote server ##########################################

'param([string]$pkgid)' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$machinename = $env:COMPUTERNAME' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$datetime = Get-Date -Format "yyyyMMdd";' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$nomadpackages = Get-WmiObject nomadpackages | Where-Object {$_.packageid -eq "$pkgid"}' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$pkgdisksize = $nomadpackages.diskusagekb' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$pkgelapsed = $nomadpackages.elapsedseconds' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$pkgreturnstate = $nomadpackages.returnstatus' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$pkgbytespeer = $nomadpackages.bytesfrompeer' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$pkgbytesdp = $nomadpackages.bytesfromdp' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$pkgstarttime = $nomadpackages.starttimeutc' | out-file -append \\$fserver\c$\upload\ncm.ps1

'$pkgpercent = $nomadpackages.percent' | out-file -append \\$fserver\c$\upload\ncm.ps1

 

'reg query HKLM\Software\1E\NomadBranch > c:\upload\nomad_keys.txt' | out-file -append \\$fserver\c$\upload\ncm.ps1

'reg query HKLM\Software\Wow6432Node\1E\PXELiteServer > c:\upload\pxe_keys.txt' | out-file -append \\$fserver\c$\upload\ncm.ps1

'reg query HKLM\Software\1E\NomadBranch\ActiveEfficiency > c:\upload\ae_keys.txt' | out-file -append \\$fserver\c$\upload\ncm.ps1

 

'"$machinename,$pkgdisksize,$pkgelapsed,$pkgreturnstate,$pkgbytespeer,$pkgbytesdp,$pkgstarttime,$pkgpercent" | out-file c:\upload\$machinename-$pkgid-$datetime.csv' | out-file -append \\$fserver\c$\upload\ncm.ps1
...
[string]$batchdone = read-host  "Type done here when done.:"

# this pause is put in to wait till the batch file is done.

if(!($batchdone -eq "done")){

[string]$batchdone = read-host "Type done here when done. No other inputs accepted. Stop the script manually otherwise:"
...
xcopy \\$rserver\c$\upload\*_keys.txt "$ncmdir\workstations\$rserver\$datetime"
...
if(test-path("\\$rserver\c$\upload\$rserver-$pkgid-$datetime.csv")){

get-content \\$rserver\c$\upload\$rserver-$pkgid-$datetime.csv | out-file -append $ncm\good\$pkgid-$datetime-cachereport.csv
